...
import { X, Loader2, Copy, Check, Plus, Trash } from 'lucide-react';
...
const APIGeneration = () => {
  ...
  const handleSetupPayment = async () => {
    try {
      ...
      // Instead of initiating payment directly, open the credit purchase modal
      setBuyCreditsOpen(true);
    } catch (err) {
      console.error('Setup payment failed:', err);
      setError(
        `Setup payment failed: ${err instanceof Error ? err.message : 'Unknown error'}`,
      );
    }
  };
  ...
  const handleBuyCredits = async () => {
    try {
      ...
      // Amount is directly in dollars now
      ...
      setError(
        `Credits purchase failed: ${err instanceof Error ? err.message : 'Unknown error'}`,
      );
    }
  };
  ...
  const handleBuyCreditsSuccess = async (response: any, amount: number) => {
    try {
      ...
      if (verifyResponse.status === 200) {
        // Update user's credits (logic unchanged for additional credits purchase)
        // const { session, user } = await getSessionAndUser();
        // if (session && user) {
        //   const { error: updateError } = await supabase
        //     .from('users')
  ...
  const generateApiKey = async () => {
    try {
      ...
      if (!userPlan || userPlan !== 'active' || credits <= 0) {
        setError(
          'Insufficient credits or invalid plan. Please upgrade your plan.',
        );
  ...
  const deleteApiKey = async () => {
    try {
      ...
      // Clear the API key from state on success.
      setApiKey('');
    } catch (err: any) {
  ...
  return (
    <div className="bg-[#171717] py-8">
      ...
      <div className="container mx-auto px-4 mb-8">
        <nav className="flex flex-col md:flex-row justify-between items-center bg-[#131010] shadow rounded p-4">
          <div className="flex space-x-6 mb-4 md:mb-0">
            <Link
              href="https://docs.thepotatoai.com"
      ...
      {!hasPaymentSetup && (
        <div className="container mx-auto px-4 mb-8">
          <div className="flex flex-col md:flex-row items-center justify-between bg-[#131010] p-6 rounded shadow">
            <div>
                  ...
                  {!apiKey ? (
                    ...
                    <div className="space-y-2 w-full">
                      <div className="p-4 bg-black rounded-lg break-all flex items-center">
                        <p className="font-mono text-sm text-white flex-grow">
                          {apiKey}
                        </p>
                        <button
          ...
          <div className="bg-[#33363D] rounded-lg p-6 max-w-md w-full mx-auto">
            ...
            <div className="mb-4">
              ...
              {/* <input
                ...
                value={creditsAmount}
                onChange={(e) =>
                  setCreditsAmount(Math.max(2, parseInt(e.target.value) || 2))
                }
