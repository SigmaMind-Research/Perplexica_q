...
interface ChatModelType {
  chatModelProviders: {
    [key: string]: { name: string; displayName: string }[]; // Define the type for the models
  };
}
...
const ChatModel = ({
  ...
  setIsChatModelOpen: (isOpen: boolean) => void;
}) => {
  const [config, setConfig] = useState<ChatModelType | null>(null);
  const [chatModels, setChatModels] = useState<Record<string, any>>({});
  ...
  const handleProviderChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedChatModelProvider(provider);
    const defaultModel =
      config?.chatModelProviders[provider]?.[0]?.name || '';
  ...
  return (
    <div className="flex items-center space-x-4">
      {isLoading ? (
        <p className="text-sm text-gray-500 dark:text-gray-300">Loading models...</p>
      ) : (
        <>
          <div className="flex flex-col space-y-4">
            <div className="flex items-center space-x-2">
              {/* <p className="text-lg font-medium text-black dark:text-white">Choose Model Provider:</p> */}
              {/* <Select
                value={selectedChatModelProvider ?? ''}
                onChange={handleProviderChange}
                options={Object.keys(chatModels).map((provider) => ({
                  value: provider,
                  label: provider.charAt(0).toUpperCase() + provider.slice(1),
                }))}
              /> */}
            ...
            {selectedChatModelProvider && (
              <div className="flex items-center space-x-2">
                {/* <p className="text-lg font-medium text-black dark:text-white"></p> */}
                <Select
                  value={selectedChatModel ?? ''}
                  onChange={handleModelChange}
                  options={(() => {
                    const chatModelProvider = chatModels[selectedChatModelProvider];

                    return chatModelProvider
                      ? chatModelProvider.map((model: { name: string; displayName: string }) => ({
                          value: model.name,
                          label: model.displayName,
                        }))
                      : [
          ...
          <button
            onClick={handleSave}
            className="mt-4 px-4 py-2 bg-blue-600 text-white text-xs rounded-lg hover:bg-blue-700"
          >
            Save
          </button>
        </>
