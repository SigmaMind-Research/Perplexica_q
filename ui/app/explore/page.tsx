...
// ✅ Removed unnecessary memoization since the component is simple
...
const DiscoverContent = memo(({
  ...
  const handleSummarize = (url: string) => {
    router.push(`/?q=Summary: ${encodeURIComponent(url)}`); 
  };
      ...
      {discover &&
        discover.map((item, i) => (
          <Link
            ...
            href={item.url}
            key={i}
            prefetch={false} // Prevent prefetching to force dynamic fetching
            className="max-w-sm rounded-lg overflow-hidden bg-light-secondary dark:bg-dark-secondary hover:-translate-y-[1px] transition duration-200"
            target="_blank"
          >
